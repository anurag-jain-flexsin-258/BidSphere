...
@props(['tender' => null, 'categories', 'buttonText' => 'Save Tender'])
...
<div class="row g-3">
    ...
    <div class="col-md-6">
        ...
        <input type="number" name="quantity" id="quantity" value="{{ old('quantity', $tender?->quantity ?? '') }}"
    ...
    <div class="col-md-6">
        ...
        <input type="date" name="expires_at" id="expires_at"
            value="{{ old('expires_at', $tender?->expires_at?->format('Y-m-d') ?? '') }}" class="form-control">
    </div>
        ...
        <div class="">
            @php
                $selectedCategories = old(
                    'categories',
                    $tender?->categories?->pluck('id')->toArray() ?? []
                );
            @endphp

            <x-category-tree :categories="$categories" :selected="$selectedCategories" />
        </div>
    </div>
    ...
    <div class="col-12">
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="is_featured" value="1" id="is_featured" {{ old('is_featured', $tender?->is_featured ?? false) ? 'checked' : '' }}>
